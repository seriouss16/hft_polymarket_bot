...
from core.live_common import BUY, SELL_SIDE, OrderStatus, TrackedOrder
...
async def test_critical_path_integration_full_cycle(mock_env):
    ...
    order_id = "order_123"
    with (
        patch.object(engine, "_place_limit_raw", return_value=(order_id, False)) as mock_place,
...
async def test_anti_doubling_safety(mock_env):
    ...
    engine._active_orders["order_1"] = TrackedOrder(
        order_id="order_1", token_id=token_id, side=BUY, price=0.50, size=10.0, status=OrderStatus.PENDING
    )
...
async def test_circuit_breaker_integration(mock_env):
    ...
    """
    with (
        patch("core.live_engine.ClobClient", MagicMock()) as mock_clob,
    ...
    with patch("core.live_engine.ClobClient", engine.client), patch("core.live_engine.OrderArgs", MagicMock()):
        for i in range(5):
...
async def test_kill_switch_integration(mock_env):
    ...
    tracked = TrackedOrder(
        order_id=order_id, token_id=token_id, side=BUY, price=0.50, size=10.0, status=OrderStatus.PENDING
    )
